...
var minimumTotalDistance = function (robot, factory) {
  ...
  for (let i = robotCount - 1; i >= 0; i--) {
    const current = new Array(factoryCount + 1).fill(0);
    // No factories left case
    if (i !== robotCount - 1) next[factoryCount] = Number.MAX_SAFE_INTEGER;
    // Initialize current row
    current[factoryCount] = Number.MAX_SAFE_INTEGER;

    for (let j = factoryCount - 1; j >= 0; j--) {
      // Option 1: Assign current robot to current factory
      const assign = Math.abs(robot[i] - factoryPositions[j]) + next[j + 1];
