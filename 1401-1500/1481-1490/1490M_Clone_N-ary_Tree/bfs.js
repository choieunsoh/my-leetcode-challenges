...
var cloneTree = function (root) {
  ...
  const clonedRoot = new _Node(root.val);
  const stack = [[root, clonedRoot]];
  while (stack.length) {
    const [originalNode, clonedNode] = stack.pop();
    for (const child of originalNode.children) {
      const clonedChild = new _Node(child.val);
      clonedNode.children.push(clonedChild);
      stack.push([child, clonedChild]);
    }
  }
  return clonedRoot;
};
