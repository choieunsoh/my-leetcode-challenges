...
var gearRatios = function (input) {
  ...
  for (let i = 0; i < rows; i++) {
    for (let j = 0; j < cols; j++) {
      ...
      if (isNumber(ch)) {
        ...
        const [valid, key] = isValid(i, start, end);
        if (valid && key) {
          if (!map.has(key)) map.set(key, []);
          map.get(key).push(Number(num));
        }
        num = '';
        start = null;
        end = null;
  ...
  for (const [, list] of map) {
    if (list.length > 1) {
      result += list.reduce((total, num) => total * num, 1);
    }
  }
  return result;
  ...
  function isValid(row, col1, col2) {
    const left = grid[row][col1 - 1] ?? '.';
    if (!isNumber(left) && left !== '.') {
      return [true, left === '*' ? `${row}-${col1 - 1}` : ''];
    }

    const right = grid[row][col2 + 1] ?? '.';
    if (!isNumber(right) && right !== '.') {
      return [true, right === '*' ? `${row}-${col2 + 1}` : ''];
    }

    const _rows = [Math.max(0, row - 1), Math.min(row + 1, rows - 1)];
    for (const i of _rows) {
      for (let j = Math.max(0, col1 - 1); j <= Math.min(col2 + 1, cols - 1); j++) {
        const ch = grid[i][j];
        if (!isNumber(ch) && ch !== '.') {
          return [true, ch === '*' ? `${i}-${j}` : ''];
        }
...
...$.*....
.664.598..`;
var result = gearRatios(input);
console.log(result);

var input = `...............776..............552........968..................589...26...........484..............958......186....546.........484.........
.........*.........778....................*....124...................................*...............*........%..26.........................
