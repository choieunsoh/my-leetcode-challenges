...
var cubeConundrum = function (input) {
  const colors = {
    red: 0,
    green: 1,
    blue: 2,
  ...
  for (const line of lines) {
    const power = possible(line);
    result += power;
  }
  return result;

  function possible(line) {
    const counts = new Array(3).fill(0);
    let valid = true;
    const [gameText, bagText] = line.split(':');
    const [, gameNo] = gameText.split(' ');
    const bags = bagText.split(';');
    for (let i = 0; valid && i < bags.length; i++) {
      const bag = bags[i];
      const cubes = bag.split(',').map((s) => s.trim());
      for (let j = 0; valid && j < cubes.length; j++) {
        const cube = cubes[j];
        const cubeToken = cube.split(' ');
        const count = Number(cubeToken[0]);
        const color = cubeToken[1];
        const index = colors[color];
        counts[index] = Math.max(counts[index], count);
      }
    }
    return counts.reduce((total, num) => total * num, 1);
  }
};

var result = cubeConundrum(input);
console.log(result);
